# 그래프 이해
"""
1. 그래프(Graph) : 실제 세계의 현상이나 사물을 정점(Vertex) 또는 노드(Node)와 간선(Edge)으로 표현하기 위해 사용

2. 그래프 관련 용어
- 노드(Node): 위치를 말함
- 간선(Edge): 위치 간의 관계를 표시한 선으로 노드를 연결한 선
- 인접 정점: 간선으로 직접 연결된 노드
...
3. 그래프 종류
무방향 그래프 : 방향이 없는 그래프, 간선을 통해 노드는 양방향으로 갈 수 있음, (A, B) / (B, A)로 표기
방향 그래프 : 간선에 방향이 있는 그래프, A -> B <A, B>로 표기 / B -> A <B, A>로 표기 <A, B> != <B, A> 
